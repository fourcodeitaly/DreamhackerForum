...
export function CommentSection({ postId, initialComments = [] }: CommentSectionProps) {
  ...
  const fetchComments = async (sort: CommentSortType = "top", pageNum = 1, append = false) => {
    if (pageNum === 1) {
      setIsLoading(true)
    } else {
  ...
  const handleCommentDelete = (commentId: string) => {
    setComments((prev) =>
      prev.map((comment) =>
        comment.id === commentId ? { ...comment, content: "[deleted]", status: "deleted" } : comment,
      ),
    )
  }
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border-2 border-destructive/20 bg-card p-6 text-center">
          <div className="flex flex-col items-center gap-3">
            ...
            <Button onClick={() => fetchComments(sortType)} variant="outline" size="sm" className="mt-2">
