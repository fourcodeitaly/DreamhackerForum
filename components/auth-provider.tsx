...
import { getUserData, loginUser, registerUser, resendConfirmationEmailServer, logoutUser } from "@/app/actions/auth"
    ...
    const initAuth = async () => {
      try {
        ...
        if (authDataScript) {
          try {
            ...
            if (serverAuthData.isAuthenticated) {
              const { user: userData, error } = await getUserData(serverAuthData.userId)
              if (userData && !error) {
                setUser(userData)
                setIsLoading(false)
                return
        ...
        const {
          ...
          if (session?.user) {
            const { user: userData, error } = await getUserData(session.user.id)
            if (userData && !error) {
              setUser(userData)
            } else {
              setUser(null)
