...
export function PostList({ initialPosts = [], totalPosts = 0, currentPage = 1, page, sort, category }: PostListProps) {
    ...
    <div>
      {isLoading ? (
        <div className="space-y-6">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="bg-gray-100 dark:bg-gray-800 animate-pulse h-40 rounded-lg"></div>
          ))}
        </div>
      ) : (
        <div className="space-y-6">
