...
export function RelatedPosts({ posts }: RelatedPostsProps) {
  ...
  return (
    <div className="mt-8">
      <h2 className="text-2xl font-bold mb-4">{t("relatedPosts") || "Related Posts"}</h2>
        ...
        {posts.map((post) => {
          ...
          const title =
            typeof post.title === "object"
              ? post.title?.[language] || post.title?.en || t("untitledPost") || "Untitled Post"
            ...
            <Link href={`/posts/${post.id}`} key={post.id}>
              <Card className="h-full hover:bg-accent/50 transition-colors">
                ...
                <CardFooter className="p-4 pt-0">
                  <p className="text-sm text-muted-foreground">{t("readMore") || "Read more"}</p>
                </CardFooter>
              </Card>
            </Link>
          )
