...
export async function POST(request: NextRequest) {
  try {
    ...
    if (success) {
      return NextResponse.json({ success: true, message: "User set as admin successfully" })
    } else {
      return NextResponse.json({ success: false, message: "Failed to set user as admin" }, { status: 500 })
    }
  } catch (error) {
