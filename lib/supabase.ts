...
let serverSupabaseClient: ReturnType<typeof createClient<Database>> | null =
  null;
...
export const createSafeClientSupabaseClient = () => {
  try {
    ...
    if (typeof window === "undefined") {
      console.warn(
        "createSafeClientSupabaseClient was called on the server side. Using server client instead."
      );
